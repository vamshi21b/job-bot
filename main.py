...
def inject_cookies(context):
    ...
    else: print("⚠️ INDEED_COOKIES environment variable is missing or empty.")
    ...
    else: print("⚠️ ZIP_COOKIES environment variable is missing or empty.")
...
def login_to_portals(page):
    ...
    if DICE_USERNAME:
        ...
        except Exception as e: print(f"❌ Dice Login Failed: {e}")

    if MONSTER_USERNAME:
        try:
            print("-> Attempting Monster Login...")
        ...
        except Exception as e: print(f"❌ Monster Login Failed: {e} (Cloudflare block likely)")
...
def gather_job_urls(page):
    ...
    for url in dice_urls:
        print(f"-> 🔍 Scraping Dice Queue: {url}")
        try:
            ...
            for link in links:
                ...
                if href: 
                    master_queue.append(("Dice", href.split('?')[0] if href.startswith('http') else f"https://www.dice.com{href.split('?')[0]}"))
                    found_count += 1
            print(f"   [Found {found_count} links on Dice]")
        except Exception as e: print(f"   [Error scraping Dice]: {e}")

    # 2. ZIPRECRUITER QUEUE
    zip_urls = [
    ...
    for url in zip_urls:
        print(f"-> 🔍 Scraping ZipRecruiter Queue: {url}")
        try:
            page.goto(url, timeout=30000)
            time.sleep(4)
            ...
            for link in links:
                href = link.get_attribute('href')
                if href: 
                    master_queue.append(("ZipRecruiter", href.split('?')[0]))
                    found_count += 1
            print(f"   [Found {found_count} links on ZipRecruiter]")
        except Exception as e: print(f"   [Error scraping ZipRecruiter]: {e} (Likely IP Blocked)")
    ...
    for url in indeed_urls:
        print(f"-> 🔍 Scraping Indeed Queue: {url}")
        try:
            page.goto(url, timeout=30000)
            time.sleep(4)
            ...
            for link in links:
                ...
                if href: 
                    ...
                    found_count += 1
            print(f"   [Found {found_count} links on Indeed]")
        except Exception as e: print(f"   [Error scraping Indeed]: {e} (Likely IP Blocked)")
    ...
    for url in linkedin_urls:
        print(f"-> 🔍 Scraping LinkedIn Queue: {url}")
        try:
            page.goto(url, timeout=30000)
            time.sleep(4)
            ...
            for link in links:
                href = link.get_attribute('href')
                if href: 
                    master_queue.append(("LinkedIn", href.split('?')[0]))
                    found_count += 1
            print(f"   [Found {found_count} links on LinkedIn]")
        except Exception as e: print(f"   [Error scraping LinkedIn]: {e}")
    ...
    for url in monster_urls:
        print(f"-> 🔍 Scraping Monster Queue: {url}")
        try:
            page.goto(url, timeout=30000)
            time.sleep(4)
            ...
            for link in links:
                ...
                if href: 
                    ...
                    found_count += 1
            print(f"   [Found {found_count} links on Monster]")
        except Exception as e: print(f"   [Error scraping Monster]: {e}")
